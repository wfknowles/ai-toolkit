...
**Concept(s) Guidance**
One of the typical tasks in the lifecycle of maintaing software is rountine maintenance like keeping dependencies up to date. Could you think of ways we could create a prompt that would be maximally helpful for a Jr Software Engineer? Explaining things is kinda boring... Im thinking how do we create a "tool" that could be used to manage and carry out the update of any software's dependencies. I think we'd need somewhere to inform the prompt on the type of dependency ("gems", "packages", etc) and which dependency manager the project utilizes (could we infer all this?). How do we do it safely? How do we determine which gems should be updated? How do we determine the latest stable version? What if the engineer wants to omit certain dependencies from being updated or how might they pin a specific version? How do we develop a methodology or framework that can accurately and precisely resolve versioning conflicts? How do we ensure we dont create any circular dependencies or create the least amount of change within the codebase as possible? Give me your best ideas!
